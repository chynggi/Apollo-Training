...
def process_audio(args, audio_name, all_files):
    print(f"[{all_files.index(audio_name) + 1}/{len(all_files)}]Processing {audio_name}")
...
def generate_dataset(args):
    ...
    with ThreadPoolExecutor(max_workers=args.threads) as executor:
        futures = [
            executor.submit(process_audio, args, audio_name, all_files)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-th", "--threads", type=int, default=cpu_count(), help="Number of threads to use for processing.")
